...
## Inputs to CRASHS
The main input to CRASHS is a medial temporal lobe (MTL) segmentation generated by [original ASHS](https://github.com/pyushkevich/ashs) or newer hyper-resolution ASHS ([hrASHS](https://github.com/pyushkevich/hyperresashs)). Before running CRASHS, you will need to run ASHS/hrASHS on your MRI scans using one of the atlases for which a CRASHS template is available. 

CRASHS offers different templates for different ASHS and hrASHS versions. Currently, the following templates are provided:
...
* **ashs_pmc_t1ext**: Template for the T1-weighted MRI version of ASHS [T1-ASHS](https://doi.org/10.1002/hbm.24607) using the [ASHS-PMC-T1ext atlas](https://www.nitrc.org/frs/?group_id=370). This atlas extends the MTL cortical structures (ERC, BA35, BA36) more anteriorly and also includes the amygdala and white matter labels.  
...
The table below provides a more specific reference


| CRASHS template | ASHS or hrASHS Atlas     | Required Inputs to CRASHS |
|-----------------|--------------------------|---------------------------|
|ashs_pmc_t1      | ASHS: ASHS-PMC-T1        | P/S, \[A\], MPR           |
|ashs_pmc_t1ext   | ASHS: ASHS-PMC-T1ext     | P/S                       |
|ashs_pmc_t1ext   | hrASHS: PMC_T1_Atlas     | S, \[A\]                  |

Abbreviations: P: posteriors, S: segmentation, A: affine_to_template, MPR: mprage
